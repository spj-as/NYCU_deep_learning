...
def train(train_loader, test_loader, epochs, model, loss_function, optimizer, mode):
    ...
    for epoch in range(epochs):
        ...
        for i, data in enumerate(tqdm(train_loader)):
            ...
            if best_acc < accuracy:
                torch.save(model.state_dict(), name + "_resnet18best.pt")
                best_acc = accuracy
    ...
    with open("ResNet18" + name + "_train_acc.txt", "w") as f:
        for item in train_acc:
            f.write(str(item) + "\n")
    with open("ResNet18" + name + "_test_acc.txt", "w") as f:
        for item in test_acc:
            f.write(str(item) + "\n")
    plot_confusion(label_, predict_, "ResNet18")
