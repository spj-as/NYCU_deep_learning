...
def loss_function(y, y_pred):
    ...
    # return -np.mean(np.sum(y * np.log(y_pred + 1e5) + (1 - y) * np.log(1 - y_pred + 1e5)))
    # loss=-(1/y.shape[1])*(y@np.log(y_pred+1e3).T+(1-y)@np.log(1-y_pred+1e3).T)
    # return float(loss)
...
class NNModel:
    def __init__(self, lr, hidden_size, activation_function, bias_exist, momentum):
        self.x = None
        self.z = [
    ...
    def backward(self, y, y_pred):
        ...
        # update the weights
        self.v_weight[0] = self.momentum * self.v_weight[0] + self.lr * dloss_dweight0.T
        self.weight[0] -= self.v_weight[0]
        self.v_weight[1] = self.momentum * self.v_weight[1] + self.lr * dloss_dweight1.T
        self.weight[1] -= self.v_weight[1]
        self.v_weight[2] = self.momentum * self.v_weight[2] + self.lr * dloss_dweight2.T
        self.weight[2] -= self.v_weight[2]

        if self.bias_exist:
            self.v_bias[0] = self.momentum * self.v_bias[0] + self.lr * dloss_dbias0
            self.bias[0] -= self.v_bias[0]
            self.v_bias[1] = self.momentum * self.v_bias[1] + self.lr * dloss_dbias1
            self.bias[1] -= self.v_bias[1]
            self.v_bias[2] = self.momentum * self.v_bias[2] + self.lr * dloss_dbias2
            self.bias[2] -= self.v_bias[2]
